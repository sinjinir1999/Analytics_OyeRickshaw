...
from
(
select extract(months from ob_date) months,count(distinct vbn) as total_vbn,count(distinct case when tat between 0 and 2 then vbn end) as zero_to_2_days,
...
from
(
Select date(onboarding_date) as ob_date,vehicle_consolidated.vbn,vc_last_call_date,(least(first_ride,first_delivery_date,first_swap_date)-date(onboarding_date)) tat
